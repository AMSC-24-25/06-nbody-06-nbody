...
namespace fmm
{
    ...
    double sign_fun1(const int k, const int m)
    {
        ...
        throw std::logic_error("Exponent in sign_fun1() is not expected to "
                               " be odd. Got input: k = " +
                               std::to_string(k) + ", m = " + std::to_string(m) + ", exponent is " + std::to_string(exponent) + "\n");
    }
    ...
    std::vector<Body_<d>> readSourcesFromFile(std::string filename)
    {

        std::ifstream ifile(filename);
        std::string line;

        std::vector<fmm::Body_<d>> sources;
    ...
    template <std::size_t d>
    void sourceLocationsToFile(std::vector<Body_<d>> sources,
