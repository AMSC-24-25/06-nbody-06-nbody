    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            if line.startswith('#'):
                ...
                continue
            try:
                particle_id, x, y = map(float, line.strip().split())
...
def setup_plot():
    colors = [
        ...
        '#e377c2', '#7f7f7f', '#17becf', '#ff7f0e', '#bcbd22'
    ]
...
def plot_frame(ax, positions, particle_trajectories, colors, frame, limits):
    ...
    for i, pos in enumerate(positions):
        ...
        ax.scatter(pos[0], pos[1], color=colors[i % len(colors)], s=100, alpha=1.0,
                  label=f'Particle {i}', edgecolor='white', linewidth=1)
...
def save_visualization(filename, fps=20, interval=800):
    ...
    def update(frame):
        if frame in timesteps:
            plot_frame(ax, timesteps[frame], particle_trajectories, colors, frame, limits)
        return ax,
    
    timestep_keys = sorted(timesteps.keys())
    anim = FuncAnimation(fig, update, frames=timestep_keys,
